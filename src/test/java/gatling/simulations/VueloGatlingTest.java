 ...
 */
public class VueloGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all vuelos").get("/api/vuelos").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new vuelo")
                        .post("/api/vuelos")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"numeroVuelo\": \"SAMPLE_TEXT\"" +
                                ", \"origen\": \"SAMPLE_TEXT\"" +
                                ", \"destino\": \"SAMPLE_TEXT\"" +
                                ", \"fechaSalida\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"fechaLlegada\": \"2020-01-01T00:00:00.000Z\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_vuelo_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created vuelo").get("${new_vuelo_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created vuelo").delete("${new_vuelo_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Vuelo entity").exec(scn);
