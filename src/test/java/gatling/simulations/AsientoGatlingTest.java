...
public class AsientoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all asientos").get("/api/asientos").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new asiento")
                        .post("/api/asientos")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" + "\"numero\": \"SAMPLE_TEXT\"" + ", \"clase\": \"SAMPLE_TEXT\"" + ", \"disponible\": null" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_asiento_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created asiento").get("${new_asiento_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created asiento").delete("${new_asiento_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Asiento entity").exec(scn);
