 ...
 */
public class ReservaGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all reservas").get("/api/reservas").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new reserva")
                        .post("/api/reservas")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"codigo\": \"SAMPLE_TEXT\"" +
                                ", \"fechaReserva\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"estado\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_reserva_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created reserva").get("${new_reserva_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created reserva").delete("${new_reserva_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Reserva entity").exec(scn);
